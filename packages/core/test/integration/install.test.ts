...
const COMBINATIONS = [
  ...
  { adapterId: 'copilot', scope: 'project', expectedSubdir: '.github/copilot/skills' },
...
describe.each(COMBINATIONS)('install: $adapterId/$scope', ({
  ...
  it.todo('fresh install: .skill-meta.json contains all required fields with correct formats');
  ...
  it.todo('drift detection: detectDrift() returns "unknown" when no .skill-meta.json exists');
  ...
  it.todo('hooks: afterInstall is called after .skill-meta.json is written');
  it.todo('hooks: both hooks receive correct arguments');
});

describe('copilot user-scope rejection', () => {
  it.todo('calling performInstall with copilot adapter and scope:user throws before writing files');
});
