...
- [ ] 2.1 Add `license?: string` to the `PackageJson` interface (lines 110–118 of `packages/create/src/detect.ts`)
- [ ] 2.2 Add `license: string` to the `DetectionResult` interface (after `description`, before `hasPackageJson`)
- [ ] 2.3 In `detectEnvironment()`, declare `let license = ''` alongside the other `package.json` locals
- [ ] 2.4 Inside the `package.json` parse block, assign `license = pkg.license ?? ''`
- [ ] 2.5 Include `license` in the returned object literal of `detectEnvironment()`
- [ ] 2.6 Run detect tests and confirm 1.1–1.3 pass

## 3. prompts.ts Implementation

- [ ] 3.1 In `collectConfig()`, change the License prompt default from `'MIT'` to `detected.license || 'MIT'` (line 57 of `packages/create/src/prompts.ts`)
- [ ] 3.2 Run prompts tests and confirm 1.4–1.5 pass
