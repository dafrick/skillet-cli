...
The `@skillet/core` package SHALL live at `packages/core/` with its own `package.json` (`"name": "@skillet/core"`, `"type": "module"`, `"engines": { "node": ">=18" }`), `tsconfig.json`, and `vitest.config.ts`.
