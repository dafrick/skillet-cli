...
`.github/workflows/ci.yml` SHALL trigger on `pull_request` and `push` to `main`. It SHALL pin Node.js to version 20 and pnpm to version 9 for reproducibility.
...
`.github/workflows/release.yml` SHALL trigger on tags matching `v*`. It SHALL be gated on `ci.yml` passing (via `workflow_run` or manual `needs`). The release job SHALL: run `pnpm install`, build `@skillet/core`, run `prepublishOnly` (which executes build + tests), and publish to npm with `--access public` under the `@skillet` scope. The npm auth token SHALL be read from the `NPM_TOKEN` repository secret.
