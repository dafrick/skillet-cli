...
- **`.skill-meta.json` manifests** — written into each install directory recording provenance, hashes, and timestamps
...
- `install-orchestration`: Resolve targets (detect or explicit), render, copy tree, write `.skill-meta.json` manifest, compute `postInstallHash`
