...
- [ ] 2.1 In `packages/create/test/unit/skill-dir.test.ts`, flip the existing assertion that `scripts/` is NOT pre-selected so it asserts `scripts/` IS pre-selected
...
- [ ] 3.1 In `packages/create/src/skill-dir.ts`, remove the `SKILL_DIRS` constant
